...
SELECT sum(pg_lip_bloom_add(1, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A4163,A6362,B6563,F6362,F6524,F6526,F6532,G6216,H6163,R1635,R1636,R2632,W4362}'::text[])));
