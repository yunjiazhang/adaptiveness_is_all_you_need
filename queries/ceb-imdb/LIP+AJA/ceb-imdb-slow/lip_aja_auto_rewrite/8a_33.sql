...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Documentary,Fantasy,Mystery,War,Western}'::text[]))) AND ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Documentary,Fantasy,Mystery,War,Western}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_cf)::text = ANY ('{B6261,M6256,M6352,P6252,R1632,R2632,S4153,S5362}'::text[])));
