...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 1) AND (mi1.info = ANY ('{10,11,12,15,20,3,30,4,5,6,60,7,8,9,90,Argentina:60,USA:60}'::text[]))) AND ((mi1.info_type_id = 1) AND (mi1.info = ANY ('{10,11,12,15,20,3,30,4,5,6,60,7,8,9,90,Argentina:60,USA:60}'::text[])));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{A4262,D5252,G6212,J5212,M2436,M6242,M6352,P425,S3126,T5214}'::text[])));
