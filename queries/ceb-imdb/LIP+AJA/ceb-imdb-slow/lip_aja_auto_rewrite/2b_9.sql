...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"Los Angeles, California, USA","Paris, France"}'::text[])) AND (mi1.info_type_id = 18)) AND ((mi1.info = ANY ('{"Los Angeles, California, USA","Paris, France"}'::text[])) AND (mi1.info_type_id = 18));
