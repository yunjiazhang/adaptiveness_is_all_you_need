...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"OFM:16 mm",OFM:Video,PCS:Spherical,PFM:Video,"RAT:1.66 : 1","RAT:1.78 : 1 / (high definition)"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"OFM:16 mm",OFM:Video,PCS:Spherical,PFM:Video,"RAT:1.66 : 1","RAT:1.78 : 1 / (high definition)"}'::text[])));
...
WHERE 
 t.id = mi1.movie_id 
   ...
   AND (mii1.info ~ '^(?:[1-9]\d*|0)?(?:\.\d+)?$' 
        AND 500.0 <= mii1.info::float) 
   AND (mii1.info ~ '^(?:[1-9]\d*|0)?(?:\.\d+)?$' 
        AND mii1.info::float <= 7200.0) 
;