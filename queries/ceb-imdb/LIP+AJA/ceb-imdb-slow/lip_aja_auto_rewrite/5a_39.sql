...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{12,4,6,8,9,90,Argentina:60,"Buenos Aires, Federal District, Argentina"}'::text[])) AND (mi1.info = ANY ('{12,4,6,8,9,90,Argentina:60,"Buenos Aires, Federal District, Argentina"}'::text[]));
