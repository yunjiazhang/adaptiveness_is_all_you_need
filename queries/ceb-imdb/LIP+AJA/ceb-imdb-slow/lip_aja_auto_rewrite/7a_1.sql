...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 1) AND (mi1.info = ANY ('{100,16,18,22,23,24,45,75,80,85,92,95,USA:30}'::text[]))) AND ((mi1.info_type_id = 1) AND (mi1.info = ANY ('{100,16,18,22,23,24,45,75,80,85,92,95,USA:30}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_nf)::text = ANY ('{A5351,A5354,C6416,C6432,F6325,I5263,J5325,J62,M6243,O4162,R2635,S3162}'::text[])));
