...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Argentina:13,Argentina:Atp,Australia:M,Canada:14A,Canada:PG,Norway:15,UK:15,UK:U}'::text[]))) AND ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Argentina:13,Argentina:Atp,Australia:M,Canada:14A,Canada:PG,Norway:15,UK:15,UK:U}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((n.name_pcode_nf IS NULL) AND ((n.gender)::text = 'f'::text));
...
WHERE 
 t.id = ci.movie_id 
   ...
   AND (cn.name IN ('Paramount Pictures', 
                    'Universal Pictures', 
                    'Warner Home Video')) 
