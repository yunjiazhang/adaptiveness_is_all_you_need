...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"MET:300 m","MET:600 m",PCS:Spherical,"PFM:35 mm","RAT:1.33 : 1",USA:Approved}'::text[])) AND (mi1.info = ANY ('{"MET:300 m","MET:600 m",PCS:Spherical,"PFM:35 mm","RAT:1.33 : 1",USA:Approved}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND (((n.name_pcode_nf)::text = ANY ('{A4163,A5362,C6421,C6423,C6424,E6523,F6521,F6524,F6525,R1631,R1632}'::text[])) OR (n.name_pcode_nf IS NULL)));
