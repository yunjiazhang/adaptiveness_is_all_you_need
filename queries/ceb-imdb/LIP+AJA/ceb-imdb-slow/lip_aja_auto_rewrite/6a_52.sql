...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Australia,"Hong Kong",Mexico,Philippines,"Soviet Union",Spain,Yugoslavia}'::text[])) AND (mi1.info = ANY ('{Australia,"Hong Kong",Mexico,Philippines,"Soviet Union",Spain,Yugoslavia}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{A6362,B6514,G6256,J25,J5243,J5263,M2453,M4245,M6241,S3151,S3153,T5252}'::text[])));
