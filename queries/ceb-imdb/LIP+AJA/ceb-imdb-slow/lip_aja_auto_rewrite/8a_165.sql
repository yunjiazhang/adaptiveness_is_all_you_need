...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Canada:13+,Canada:14A,Finland:K-12,Germany:16,Netherlands:12,Netherlands:16,Netherlands:AL,Singapore:M18,"South Korea:15","South Korea:18",Spain:18,Sweden:11,UK:18,UK:A,USA:TV-G}'::text[]))) AND ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Canada:13+,Canada:14A,Finland:K-12,Germany:16,Netherlands:12,Netherlands:16,Netherlands:AL,Singapore:M18,"South Korea:15","South Korea:18",Spain:18,Sweden:11,UK:18,UK:A,USA:TV-G}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = ANY ('{f,m}'::text[])) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A5352,B6161,C6231,C6235,E3632,G6252,J5216,J5235,M2425,R1632,S3152}'::text[])));
