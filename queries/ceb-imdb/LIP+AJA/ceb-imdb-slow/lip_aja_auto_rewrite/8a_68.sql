...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{SDDS,Stereo}'::text[])) AND (mi1.info_type_id = 6)) AND ((mi1.info = ANY ('{SDDS,Stereo}'::text[])) AND (mi1.info_type_id = 6));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_nf)::text = ANY ('{C6231,D5162,F6524,F6525,J5215,P436,R1635,R2632,R2635,S3152}'::text[])));
