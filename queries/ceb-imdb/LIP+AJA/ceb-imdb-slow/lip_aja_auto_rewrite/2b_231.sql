...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Brazil:12,Canada:18A,Finland:S,Germany:16,Iceland:L,Italy:T,Malaysia:U,Portugal:M/12,Portugal:M/6,USA:R}'::text[]))) AND ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Brazil:12,Canada:18A,Finland:S,Germany:16,Iceland:L,Italy:T,Malaysia:U,Portugal:M/12,Portugal:M/6,USA:R}'::text[])));
SELECT sum(pg_lip_bloom_add(2, mi2.movie_id)), sum(pg_lip_bloom_add(3, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{Germany,UK,USA}'::text[]))) AND ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{Germany,UK,USA}'::text[])));
