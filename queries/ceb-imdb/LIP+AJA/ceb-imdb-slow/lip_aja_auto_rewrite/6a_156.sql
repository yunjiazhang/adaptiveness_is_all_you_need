...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Canada,France,Italy,Japan,UK,USA,"West Germany"}'::text[]))) AND ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Canada,France,Italy,Japan,UK,USA,"West Germany"}'::text[])));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = ANY ('{B6514,D1352,E3631,G6262,J5236,J5261,J5263,L2142,M4245,M6352,S3521,W4361,W4525}'::text[])) OR (n.name_pcode_nf IS NULL)));
