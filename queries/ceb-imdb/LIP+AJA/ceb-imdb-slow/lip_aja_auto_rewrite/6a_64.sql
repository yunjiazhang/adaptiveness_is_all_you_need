...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{France,Germany,UK,USA}'::text[]))) AND ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{France,Germany,UK,USA}'::text[])));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = ANY ('{A4163,A5352,D1316,F6521,M2425,P3625,R1631,S3151}'::text[])) OR (n.name_pcode_nf IS NULL)));
