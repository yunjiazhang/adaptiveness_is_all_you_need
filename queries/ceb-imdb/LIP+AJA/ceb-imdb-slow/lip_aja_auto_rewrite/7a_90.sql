...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Filipino,Hindi,Italian,Japanese,Tagalog,Turkish}'::text[])) AND (mi1.info = ANY ('{Filipino,Hindi,Italian,Japanese,Tagalog,Turkish}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{A4163,A4253,A5352,C6425,E6523,F6521,F6525,R1632}'::text[])));
