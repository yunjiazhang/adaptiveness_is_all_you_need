...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"OFM:35 mm",OFM:Video,"PFM:35 mm"}'::text[])) AND (mi1.info = ANY ('{"OFM:35 mm",OFM:Video,"PFM:35 mm"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = ANY ('{f,m}'::text[])) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A5352,D1316,F6521,J5236,J5252,M2425,P3625,R1631,S3151}'::text[])));
