...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Animation,Biography,Crime,Fantasy,Horror,Italian,Japanese,Romance,Sci-Fi,Sport,Thriller}'::text[])) AND (mi1.info = ANY ('{Animation,Biography,Crime,Fantasy,Horror,Italian,Japanese,Romance,Sci-Fi,Sport,Thriller}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = 'C6235'::text) OR (n.name_pcode_nf IS NULL)));
