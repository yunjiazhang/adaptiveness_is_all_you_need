...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Drama,Family,Fantasy,Horror,Music,Sci-Fi,Short}'::text[]))) AND ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Drama,Family,Fantasy,Horror,Music,Sci-Fi,Short}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_nf)::text = ANY ('{A4252,B1626,C6421,F6525,J52,M2425,P3625,R1631,R1635,R1636}'::text[])));
