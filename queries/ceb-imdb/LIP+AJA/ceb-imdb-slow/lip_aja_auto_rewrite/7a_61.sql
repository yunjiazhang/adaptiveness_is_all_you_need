...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"$10,000",China,Finland,Greece,Ireland,Israel,"Paris, France",Poland,Sweden,Switzerland,"West Germany"}'::text[])) AND (mi1.info = ANY ('{"$10,000",China,Finland,Greece,Ireland,Israel,"Paris, France",Poland,Sweden,Switzerland,"West Germany"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{D1313,D5431,D5456,G4163,H526,L6523,P1426,R2532,S2153,S3163,S5242}'::text[])));
...
HashJoin(mii2 mii1 t ci rt an pi1 n)
HashJoin(mii2 mii1 t ci rt an pi1)
HashJoin(mii2 mii1 t ci rt an)
