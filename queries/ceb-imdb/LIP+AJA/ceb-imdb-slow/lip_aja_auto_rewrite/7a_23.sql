...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Austria,Czechoslovakia,Denmark,"Hong Kong",Poland,Portugal,"South Korea","Soviet Union",Sweden,Switzerland,Turkey,Yugoslavia}'::text[])) AND (mi1.info = ANY ('{Austria,Czechoslovakia,Denmark,"Hong Kong",Poland,Portugal,"South Korea","Soviet Union",Sweden,Switzerland,Turkey,Yugoslavia}'::text[]));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((n.gender IS NULL) AND (((n.name_pcode_nf)::text = ANY ('{A4163,A4253,A5362,A6532,C5321,C6231,C6235,R516,R5316,S3152,S3521}'::text[])) OR (n.name_pcode_nf IS NULL)));
