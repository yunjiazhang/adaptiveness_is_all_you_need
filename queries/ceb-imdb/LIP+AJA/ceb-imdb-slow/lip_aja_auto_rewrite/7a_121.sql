...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Australia:G,Finland:K-16,Mono,Stereo,Sweden:15}'::text[])) AND (mi1.info = ANY ('{Australia:G,Finland:K-16,Mono,Stereo,Sweden:15}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{A4253,A5352,C6423,C6426,E3632,E6523,F6523,F6526,G6216,J5216,J5242,P3624,R1636,R2634}'::text[])));
