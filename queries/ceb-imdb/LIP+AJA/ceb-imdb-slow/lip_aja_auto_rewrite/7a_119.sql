...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"Black AND White","OFM:35 mm",OFM:Video,"PFM:35 mm"}'::text[])) AND (mi1.info = ANY ('{"Black AND White","OFM:35 mm",OFM:Video,"PFM:35 mm"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND (((n.name_pcode_nf)::text = ANY ('{A4253,A5362,B6535,C6231,C6235,F6525,R1632,R2632,S3152}'::text[])) OR (n.name_pcode_nf IS NULL)));
