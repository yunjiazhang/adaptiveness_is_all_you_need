...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{PCS:Spherical,"PFM:35 mm",PFM:Video,"RAT:1.37 : 1","RAT:1.78 : 1","RAT:1.85 : 1","RAT:16:9 HD","RAT:2.35 : 1"}'::text[])) AND (mi1.info = ANY ('{PCS:Spherical,"PFM:35 mm",PFM:Video,"RAT:1.37 : 1","RAT:1.78 : 1","RAT:1.85 : 1","RAT:16:9 HD","RAT:2.35 : 1"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_nf)::text = ANY ('{A4252,A5364,G5362,G6265,G6356,J5251,M6246,P3625,T5252,V5253}'::text[])));
