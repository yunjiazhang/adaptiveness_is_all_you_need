...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = ANY ('{f,m}'::text[])) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = ANY ('{C6231,E4213,F6525,J513,M6251,M6263,P3625,R1631,R2631}'::text[])) OR (n.name_pcode_nf IS NULL)));


/*+
NestLoop(mii2 mii1 t ci rt pi1 an n mi1 kt it1 it3 it4 it5)
NestLoop(mii2 mii1 t ci rt pi1 an n mi1 kt it1 it3 it4)
NestLoop(mii2 mii1 t ci rt pi1 an n mi1 kt it1 it3)
