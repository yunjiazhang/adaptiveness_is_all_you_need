...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"Dolby Digital",Mono}'::text[])) AND (mi1.info_type_id = 6)) AND ((mi1.info = ANY ('{"Dolby Digital",Mono}'::text[])) AND (mi1.info_type_id = 6));
SELECT sum(pg_lip_bloom_add(2, mi2.movie_id)), sum(pg_lip_bloom_add(3, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{France,Germany,USA}'::text[]))) AND ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{France,Germany,USA}'::text[])));
