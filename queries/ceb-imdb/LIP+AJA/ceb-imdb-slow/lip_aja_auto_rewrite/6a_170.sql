...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"OFM:16 mm",OFM:Live,OFM:Video,"RAT:1.66 : 1","RAT:1.85 : 1","RAT:2.35 : 1"}'::text[])) AND (mi1.info = ANY ('{"OFM:16 mm",OFM:Live,OFM:Video,"RAT:1.66 : 1","RAT:1.85 : 1","RAT:2.35 : 1"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A5242,B6165,J3261,J5162,M6232,M6242,M6252,P3625}'::text[])));
