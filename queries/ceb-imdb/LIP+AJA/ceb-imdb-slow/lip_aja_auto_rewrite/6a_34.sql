...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Australia:M,UK:PG,USA:X,"West Germany:12","West Germany:16"}'::text[])) AND (mi1.info = ANY ('{Australia:M,UK:PG,USA:X,"West Germany:12","West Germany:16"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A6361,D1326,D1352,G6262,J5241,J5245,J5263,L2142,M6216,S3151}'::text[])));
