...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)), sum(pg_lip_bloom_add(1, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 4) AND (mi2.info = ANY ('{Danish,French,German,Hindi,Japanese,Norwegian,Romanian,Russian,Spanish}'::text[]))) AND ((mi2.info_type_id = 4) AND (mi2.info = ANY ('{Danish,French,German,Hindi,Japanese,Norwegian,Romanian,Russian,Spanish}'::text[])));
SELECT sum(pg_lip_bloom_add(2, mi1.movie_id)), sum(pg_lip_bloom_add(3, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Biography,Crime,Documentary,Drama,Family,Musical,Short}'::text[]))) AND ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Biography,Crime,Documentary,Drama,Family,Musical,Short}'::text[])));
