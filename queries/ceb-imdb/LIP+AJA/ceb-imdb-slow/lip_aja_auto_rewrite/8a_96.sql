...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 6) AND (mi1.info = ANY ('{DTS,Dolby,Mono}'::text[]))) AND ((mi1.info_type_id = 6) AND (mi1.info = ANY ('{DTS,Dolby,Mono}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = ANY ('{f,m}'::text[])) OR (n.gender IS NULL)) AND ((n.name_pcode_cf)::text = ANY ('{A5362,B6261,D1232,H4236,O4252,P5235,P6252,R363,S3152,W5165}'::text[])));
