...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"RAT:1.33 : 1","RAT:1.37 : 1","RAT:2.35 : 1"}'::text[])) AND (mi1.info = ANY ('{"RAT:1.33 : 1","RAT:1.37 : 1","RAT:2.35 : 1"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{C6421,C6423,C6424,C6426,F6524,R1631,R2632,V4356,W4362}'::text[])));


/*+
HashJoin(mii2 mii1 t mk ci rt pi1 an n mi1 kt it1 it3 it4 k it5)
NestLoop(mii2 mii1 t mk ci rt pi1 an n mi1 kt it1 it3 it4 k)
HashJoin(mii2 mii1 t mk ci rt pi1 an n mi1 kt it1 it3 it4)
HashJoin(mii2 mii1 t mk ci rt pi1 an n mi1 kt it1 it3)
NestLoop(mii2 mii1 t mk ci rt pi1 an n mi1 kt it1)
NestLoop(mii2 mii1 t mk ci rt pi1 an n mi1 kt)
NestLoop(mii2 mii1 t mk ci rt pi1 an n mi1)
HashJoin(mii2 mii1 t mk ci rt pi1 an n)
NestLoop(mii2 mii1 t mk ci rt pi1 an)
