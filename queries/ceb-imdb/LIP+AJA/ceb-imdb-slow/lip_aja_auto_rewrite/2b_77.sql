...
SELECT sum(pg_lip_bloom_add(2, mi1.movie_id)), sum(pg_lip_bloom_add(3, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 4) AND (mi1.info = ANY ('{French,German,Italian,Japanese,Mandarin,Portuguese,Spanish}'::text[]))) AND ((mi1.info_type_id = 4) AND (mi1.info = ANY ('{French,German,Italian,Japanese,Mandarin,Portuguese,Spanish}'::text[])));
