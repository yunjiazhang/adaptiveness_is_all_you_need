...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Filipino,Greek,Hindi,Mandarin,"OFM:16 mm","PFM:16 mm",Portuguese,"RAT:1.66 : 1","RAT:1.85 : 1",Russian,Serbo-Croatian,Tagalog}'::text[])) AND (mi1.info = ANY ('{Filipino,Greek,Hindi,Mandarin,"OFM:16 mm","PFM:16 mm",Portuguese,"RAT:1.66 : 1","RAT:1.85 : 1",Russian,Serbo-Croatian,Tagalog}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND (((n.name_pcode_nf)::text = ANY ('{B6162,C6235,C6416,C6452,E4213,R3565}'::text[])) OR (n.name_pcode_nf IS NULL)));
