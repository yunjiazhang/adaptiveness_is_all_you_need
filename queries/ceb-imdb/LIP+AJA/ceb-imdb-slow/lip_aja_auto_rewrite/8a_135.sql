...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{LAB:DeLuxe,PCS:Kinescope,PCS:Spherical,"PFM:35 mm","RAT:1.78 : 1","RAT:1.85 : 1","RAT:2.35 : 1"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{LAB:DeLuxe,PCS:Kinescope,PCS:Spherical,"PFM:35 mm","RAT:1.78 : 1","RAT:1.85 : 1","RAT:2.35 : 1"}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_cf)::text = ANY ('{A4361,A5362,B6525,B6526,M6256,P3625,P6252,R1525,R2632,S4153,W4525}'::text[])));
