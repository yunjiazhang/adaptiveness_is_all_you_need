...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Australia:M,Australia:PG,Canada:PG,Iceland:L,Singapore:PG,UK:PG}'::text[]))) AND ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Australia:M,Australia:PG,Canada:PG,Iceland:L,Singapore:PG,UK:PG}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_cf)::text = ANY ('{A5362,B6525,B6526,G6262,J5256,M6256,P3625,R1525,R2632,S1452,S4153,W4525}'::text[])));
