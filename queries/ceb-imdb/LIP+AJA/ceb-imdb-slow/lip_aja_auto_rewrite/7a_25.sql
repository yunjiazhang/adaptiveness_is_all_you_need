...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"London, England, UK",Mexico,"Toronto, Ontario, Canada"}'::text[])) AND (mi1.info = ANY ('{"London, England, UK",Mexico,"Toronto, Ontario, Canada"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{B6325,B6525,D5236,E3631,F6525,J2165,J5245,K1515,M6265,M6356,O4162}'::text[])));
