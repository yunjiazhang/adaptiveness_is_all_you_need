...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Argentina:16,Australia:MA,Canada:PG,Germany:12,Germany:16,Iceland:16,Iceland:L,"London, England, UK","Paris, France",Sweden:15,"Toronto, Ontario, Canada",USA:PG,USA:TV-PG}'::text[])) AND (mi1.info = ANY ('{Argentina:16,Australia:MA,Canada:PG,Germany:12,Germany:16,Iceland:16,Iceland:L,"London, England, UK","Paris, France",Sweden:15,"Toronto, Ontario, Canada",USA:PG,USA:TV-PG}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A2351,A4253,C6424,D2564,J216,J242,J2565,J5141,J5216,J6242,J6264,R2532,R5432}'::text[])));
