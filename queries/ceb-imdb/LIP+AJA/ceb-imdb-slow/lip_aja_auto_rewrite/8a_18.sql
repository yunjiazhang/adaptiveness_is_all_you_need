...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Argentina:Atp,Finland:S,France:U,Germany:12,Iceland:16,Singapore:PG,Sweden:15,UK:15,UK:PG,USA:PG,"West Germany:12"}'::text[]))) AND ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Argentina:Atp,Finland:S,France:U,Germany:12,Iceland:16,Singapore:PG,Sweden:15,UK:15,UK:PG,USA:PG,"West Germany:12"}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND ((n.name_pcode_cf)::text = 'P5235'::text));
