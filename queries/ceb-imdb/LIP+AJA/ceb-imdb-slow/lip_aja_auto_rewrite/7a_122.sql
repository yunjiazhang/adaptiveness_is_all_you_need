...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"OFM:35 mm",PFM:Video,"RAT:1.33 : 1"}'::text[])) AND (mi1.info = ANY ('{"OFM:35 mm",PFM:Video,"RAT:1.33 : 1"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_nf)::text = ANY ('{A4253,A5242,B6563,D1316,J5252,M2416,M2425,P3625,R1631,R1635,W4125}'::text[])));
...
HashJoin(mii2 mii1 t ci rt pi1 mi1 kt it1 it3 it4 mk k an)
HashJoin(mii2 mii1 t ci rt pi1 mi1 kt it1 it3 it4 mk k)
