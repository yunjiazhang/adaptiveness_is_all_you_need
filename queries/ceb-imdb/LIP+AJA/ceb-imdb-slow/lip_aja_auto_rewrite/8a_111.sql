...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"New York City, New York, USA","Paramount Studios - 5555 Melrose Avenue, Hollywood, Los Angeles, California, USA"}'::text[])) AND (mi1.info_type_id = 18)) AND ((mi1.info = ANY ('{"New York City, New York, USA","Paramount Studios - 5555 Melrose Avenue, Hollywood, Los Angeles, California, USA"}'::text[])) AND (mi1.info_type_id = 18));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND (((n.name_pcode_nf)::text = ANY ('{A4163,C6231,C6421,C6423,C6425,F6521,F6524,F6525,R1631,R1632,R2632}'::text[])) OR (n.name_pcode_nf IS NULL)));
