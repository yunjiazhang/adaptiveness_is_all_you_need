...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Germany,Philippines,"Soviet Union",Turkey,"West Germany",Yugoslavia}'::text[]))) AND ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Germany,Philippines,"Soviet Union",Turkey,"West Germany",Yugoslavia}'::text[])));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{C6423,C6424,C6426,F6524,F6532,R1631,R2632,V4356,W4362}'::text[])));
