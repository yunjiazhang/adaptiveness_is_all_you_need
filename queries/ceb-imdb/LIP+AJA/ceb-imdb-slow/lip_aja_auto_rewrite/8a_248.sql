...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"CAM:Panavision Cameras AND Lenses","LAB:Technicolor, Hollywood (CA), USA",MET:,"OFM:35 mm","PCS:Digital Intermediate",PCS:Kinescope,"PFM:35 mm","PFM:70 mm","RAT:1.33 : 1","RAT:16:9 HD"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"CAM:Panavision Cameras AND Lenses","LAB:Technicolor, Hollywood (CA), USA",MET:,"OFM:35 mm","PCS:Digital Intermediate",PCS:Kinescope,"PFM:35 mm","PFM:70 mm","RAT:1.33 : 1","RAT:16:9 HD"}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{B6161,C6235,K3451,M6216}'::text[])));
