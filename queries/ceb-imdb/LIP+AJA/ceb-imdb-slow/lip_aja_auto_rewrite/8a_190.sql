...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 6) AND (mi1.info = ANY ('{DTS,"Dolby Digital",Dolby,SDDS}'::text[]))) AND ((mi1.info_type_id = 6) AND (mi1.info = ANY ('{DTS,"Dolby Digital",Dolby,SDDS}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{C6235,E4213,M6216,M6263}'::text[])));
