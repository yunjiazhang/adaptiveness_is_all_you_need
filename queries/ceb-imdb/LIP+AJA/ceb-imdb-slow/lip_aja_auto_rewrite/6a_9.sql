...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Australia:G,Finland:K-16,Mono,Stereo,Sweden:15,USA:R}'::text[])) AND (mi1.info = ANY ('{Australia:G,Finland:K-16,Mono,Stereo,Sweden:15,USA:R}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{B1626,C6231,C6424,C6425,F6521,H6163,J5165,J5235,J5253,J5262,J5425,R1636}'::text[])));
