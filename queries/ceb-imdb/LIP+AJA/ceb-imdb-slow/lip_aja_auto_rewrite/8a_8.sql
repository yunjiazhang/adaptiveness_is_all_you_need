...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{LAB:DeLuxe,"OFM:35 mm",PCS:Spherical,"PFM:35 mm","RAT:1.78 : 1","RAT:1.85 : 1","RAT:16:9 HD"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{LAB:DeLuxe,"OFM:35 mm",PCS:Spherical,"PFM:35 mm","RAT:1.78 : 1","RAT:1.85 : 1","RAT:16:9 HD"}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = ANY ('{f,m}'::text[])) OR (n.gender IS NULL)) AND ((n.name_pcode_cf)::text = ANY ('{A5362,B6245,B6261,H4236,P5235,P6252,R363,W5165}'::text[])));
