...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Argentina:13,Canada:PG,France:U,Germany:12,Iceland:L,Netherlands:12,Portugal:M/12,Sweden:15,UK:PG,UK:U,USA:PG,USA:R}'::text[]))) AND ((mi1.info_type_id = 5) AND (mi1.info = ANY ('{Argentina:13,Canada:PG,France:U,Germany:12,Iceland:L,Netherlands:12,Portugal:M/12,Sweden:15,UK:PG,UK:U,USA:PG,USA:R}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'm'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_cf)::text = ANY ('{A5362,B4525,B6245,B6261,D1232,J5252,O4252,R1632,S3152,W5165}'::text[])));
