...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{100,11,13,14,16,17,22,25,3,45,95,USA:30}'::text[])) AND (mi1.info = ANY ('{100,11,13,14,16,17,22,25,3,45,95,USA:30}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND ((n.name_pcode_nf)::text = ANY ('{A4262,C6232,C6521,E4516,I6526,J5265,J5616,L2425,M4532,M6234,S4125,S5162}'::text[])));
