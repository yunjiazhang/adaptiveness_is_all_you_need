...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Austria,Czechoslovakia,Denmark,Finland,Hungary,Poland,Portugal,"South Korea","Soviet Union",Sweden,Switzerland}'::text[]))) AND ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Austria,Czechoslovakia,Denmark,Finland,Hungary,Poland,Portugal,"South Korea","Soviet Union",Sweden,Switzerland}'::text[])));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{A2352,A6543,B5252,C6253,D1325,J256,J5,J5232,J563,M5425,P3615,R54,T5214,T525}'::text[])));
