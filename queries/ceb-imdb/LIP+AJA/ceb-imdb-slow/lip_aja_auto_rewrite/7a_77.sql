...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Canada,France,Germany,UK}'::text[])) AND (mi1.info = ANY ('{Canada,France,Germany,UK}'::text[]));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{B6563,C6425,D1316,F6523,J5252,M2416,M2425,R1635,R2631}'::text[])));
