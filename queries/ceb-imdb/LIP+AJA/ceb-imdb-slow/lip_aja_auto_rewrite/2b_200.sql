...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)), sum(pg_lip_bloom_add(1, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{Canada,Germany,India,Japan,Spain,USA}'::text[]))) AND ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{Canada,Germany,India,Japan,Spain,USA}'::text[])));
SELECT sum(pg_lip_bloom_add(2, mi1.movie_id)), sum(pg_lip_bloom_add(3, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"LAB:Consolidated Film Industries (CFI), Hollywood (CA), USA","PCS:Digital Intermediate",PCS:Spherical,"PFM:16 mm","PFM:35 mm",PFM:Digital,"RAT:1.85 : 1","RAT:2.35 : 1"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"LAB:Consolidated Film Industries (CFI), Hollywood (CA), USA","PCS:Digital Intermediate",PCS:Spherical,"PFM:16 mm","PFM:35 mm",PFM:Digital,"RAT:1.85 : 1","RAT:2.35 : 1"}'::text[])));
