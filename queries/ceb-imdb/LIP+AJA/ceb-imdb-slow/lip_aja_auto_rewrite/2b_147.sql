...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"New York City, New York, USA","Sydney, New South Wales, Australia"}'::text[])) AND (mi1.info_type_id = 18)) AND ((mi1.info = ANY ('{"New York City, New York, USA","Sydney, New South Wales, Australia"}'::text[])) AND (mi1.info_type_id = 18));
SELECT sum(pg_lip_bloom_add(2, mi2.movie_id)), sum(pg_lip_bloom_add(3, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info = ANY ('{Drama,Thriller}'::text[])) AND (mi2.info_type_id = 3)) AND ((mi2.info = ANY ('{Drama,Thriller}'::text[])) AND (mi2.info_type_id = 3));
