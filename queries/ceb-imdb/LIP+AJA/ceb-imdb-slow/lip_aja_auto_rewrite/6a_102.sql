...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{11,13,14,16,17,20,4,9,Belgium,Brazil,Greece,Netherlands,Portugal,"Soviet Union",Sweden,Switzerland}'::text[])) AND (mi1.info = ANY ('{11,13,14,16,17,20,4,9,Belgium,Brazil,Greece,Netherlands,Portugal,"Soviet Union",Sweden,Switzerland}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = ANY ('{A4253,A5242,A5362,C6452,K5164,S3152}'::text[])) OR (n.name_pcode_nf IS NULL)));
