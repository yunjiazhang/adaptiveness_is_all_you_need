...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Adult,Adventure,Biography,Fantasy,History,Music,Musical,Mystery,Sci-Fi,Sport}'::text[])) AND (mi1.info = ANY ('{Adult,Adventure,Biography,Fantasy,History,Music,Musical,Mystery,Sci-Fi,Sport}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = ANY ('{f,m}'::text[])) AND ((n.name_pcode_nf)::text = ANY ('{B3625,B3626,B6164,D5251,J2612,J4524,J5262,M2424,P436,R53,S252,S5253}'::text[])));
