...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"RAT:1.78 : 1","RAT:1.85 : 1"}'::text[])) AND (mi1.info_type_id = 7)) AND ((mi1.info = ANY ('{"RAT:1.78 : 1","RAT:1.85 : 1"}'::text[])) AND (mi1.info_type_id = 7));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A4252,B6162,C6235,J5162,K3451,L5326,M6216,S3521}'::text[])));
