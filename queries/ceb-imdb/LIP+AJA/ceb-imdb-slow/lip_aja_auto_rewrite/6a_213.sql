...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{China,Czechoslovakia,Denmark,Finland,Greece,"Hong Kong",Hungary,Ireland,Poland,Portugal,"South Korea","Soviet Union",Sweden,Switzerland,Turkey,Yugoslavia}'::text[])) AND (mi1.info = ANY ('{China,Czechoslovakia,Denmark,Finland,Greece,"Hong Kong",Hungary,Ireland,Poland,Portugal,"South Korea","Soviet Union",Sweden,Switzerland,Turkey,Yugoslavia}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND (((n.name_pcode_nf)::text = ANY ('{A5365,C6462,J2363,J2532,J45,K1525,L6525,P3652,R2634,R3412,S5362}'::text[])) OR (n.name_pcode_nf IS NULL)));
