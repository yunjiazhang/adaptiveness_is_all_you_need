...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Adventure,Comedy,Drama,Music,Musical}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Action','Adventure','Comedy','Drama','Music','Musical') 
 AND mi2.info IN ('Argentina:Atp','Sweden:15','USA:Approved','USA:Passed','USA:TV-G') 
