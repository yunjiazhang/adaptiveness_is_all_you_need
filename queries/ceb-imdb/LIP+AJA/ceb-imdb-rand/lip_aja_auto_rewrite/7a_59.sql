...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Italian,Japanese,Spanish}'::text[])) AND (mi1.info = ANY ('{Italian,Japanese,Spanish}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = ANY ('{A5242,K6235}'::text[])) OR (n.name_pcode_nf IS NULL)));
...
WHERE 
 ...
 AND (mi1.info IN ('Italian','Japanese','Spanish')) 
 AND (it1.id IN ('12','4')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
 ...
 AND (n.name_pcode_nf in ('A5242','K6235') OR n.name_pcode_nf IS NULL) 
