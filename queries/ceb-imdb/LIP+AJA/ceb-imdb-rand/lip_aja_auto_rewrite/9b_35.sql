...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Action,Animation,Crime,Music,Short,Sport}'::text[])) AND (mi1.info = ANY ('{Action,Animation,Crime,Music,Short,Sport}'::text[]));
...
WHERE 
 ...
 AND (it1.id IN ('2','3','6')) 
 AND (it2.id IN ('21')) 
 AND (mi1.info IN ('Action','Animation','Crime','Music','Short','Sport')) 
 AND (n.name ILIKE '%ven%') 
