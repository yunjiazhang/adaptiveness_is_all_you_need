...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)), sum(pg_lip_bloom_add(1, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info = ANY ('{Dolby,Stereo}'::text[])) AND (mi2.info_type_id = 6)) AND ((mi2.info = ANY ('{Dolby,Stereo}'::text[])) AND (mi2.info_type_id = 6));
SELECT sum(pg_lip_bloom_add(2, mi1.movie_id)), sum(pg_lip_bloom_add(3, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 1) AND (mi1.info = ANY ('{15,22,30,60,90,Argentina:60,USA:30}'::text[]))) AND ((mi1.info_type_id = 1) AND (mi1.info = ANY ('{15,22,30,60,90,Argentina:60,USA:30}'::text[])));
...
WHERE 
 ...
 AND (k.keyword IN ('anal-sex','bare-breasts','character-name-in-title','father-son-relationship','hardcore','husband-wife-relationship','male-frontal-nudity','male-nudity','marriage','nudity','oral-sex','party','police','surrealism')) 
