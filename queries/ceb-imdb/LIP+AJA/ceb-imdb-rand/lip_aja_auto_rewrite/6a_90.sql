...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"Dolby SR",Dolby,Silent}'::text[])) AND (mi1.info = ANY ('{"Dolby SR",Dolby,Silent}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND ((n.name_pcode_nf)::text = ANY ('{A4256,A5136,C6423,C6521,D1262,E4516,L242,L2524,L652,M5216,M6541,R2416,S2512}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('Dolby SR','Dolby','Silent')) 
 AND (it1.id IN ('110','6')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
 ...
 AND (n.gender in ('f')) 
 AND (n.name_pcode_nf in ('A4256','A5136','C6423','C6521','D1262','E4516','L242','L2524','L652','M5216','M6541','R2416','S2512')) 
