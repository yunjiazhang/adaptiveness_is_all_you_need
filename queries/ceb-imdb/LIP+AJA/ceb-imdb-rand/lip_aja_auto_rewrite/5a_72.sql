...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Argentina,Greece,Italy,Mexico,Netherlands,Portugal}'::text[]))) AND ((mi1.info_type_id = 8) AND (mi1.info = ANY ('{Argentina,Greece,Italy,Mexico,Netherlands,Portugal}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('Argentina','Greece','Italy','Mexico','Netherlands','Portugal')) 
 AND (it1.id IN ('8')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
