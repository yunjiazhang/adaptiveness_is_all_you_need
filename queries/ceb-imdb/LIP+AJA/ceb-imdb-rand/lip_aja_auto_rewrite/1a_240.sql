...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Comedy,Documentary,Drama,Sci-Fi}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Comedy','Documentary','Drama','Sci-Fi') 
 AND mi2.info IN ('OFM:Video','PFM:35 mm','RAT:1.78 : 1 / (high definition)','RAT:1.85 : 1') 
