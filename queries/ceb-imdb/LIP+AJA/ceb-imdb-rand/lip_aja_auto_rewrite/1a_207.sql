...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Adult,Animation,Biography,Comedy,Documentary,Drama,Music}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Action','Adult','Animation','Biography','Comedy','Documentary','Drama','Music') 
 AND mi2.info IN ('Czech','Finnish','Georgian','German','Italian','Norwegian','Polish','Romanian','Russian','Spanish') 
