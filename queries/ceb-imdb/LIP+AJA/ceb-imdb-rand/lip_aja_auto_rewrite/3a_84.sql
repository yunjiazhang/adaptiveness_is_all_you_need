...
SELECT sum(pg_lip_bloom_add(0, cn.id)) FROM company_name AS cn 
        WHERE ((cn.country_code)::text = '[bg]'::text);
...
WHERE 
 ...
 AND (k.keyword IN ('based-on-novel','blood','doctor','father-daughter-relationship','hardcore','jealousy','lesbian','love','marriage','mother-son-relationship','number-in-title','song','suicide')) 
