...
SELECT sum(pg_lip_bloom_add(0, cn.id)) FROM company_name AS cn 
        WHERE ((cn.country_code)::text = ANY ('{[ddde],[suhh]}'::text[]));
...
WHERE 
 ...
 AND (k.keyword IN ('bare-chested-male','based-on-novel','dancing','doctor','family-relationships','father-daughter-relationship','friendship','hardcore','mother-daughter-relationship','new-york-city')) 
 AND (cn.country_code IN ('[ddde]','[suhh]')) 
