...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Crime,Documentary,Drama,Horror,Thriller}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Crime','Documentary','Drama','Horror','Thriller') 
 AND mi2.info IN ('PCS:Super 35','RAT:1.78 : 1 / (high definition)','RAT:1.78 : 1','RAT:2.35 : 1') 
