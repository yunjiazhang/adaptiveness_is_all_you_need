...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%c%'::text) AND (mi1.info_type_id = 3)) AND ((mi1.info ~~* '%c%'::text) AND (mi1.info_type_id = 3));
...
WHERE 
 ...
 AND (kt.kind ILIKE '%m%') 
 AND (rt.role IN ('actor','cinematographer','composer','costume designer','director','guest','miscellaneous crew','producer','production designer')) 
 AND (t.production_year <= 1945) 
 AND (t.production_year >= 1875) 
 AND (it.id IN ('3')) 
 AND (mi1.info ILIKE '%c%') 
 AND (cn.name ILIKE '%frè%') 
