...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Adventure,Documentary,Drama,Horror,Romance,Short}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Action','Adventure','Documentary','Drama','Horror','Romance','Short') 
 AND mi2.info IN ('English','German','Greek','Hindi','Mandarin','Serbo-Croatian','Turkish') 
