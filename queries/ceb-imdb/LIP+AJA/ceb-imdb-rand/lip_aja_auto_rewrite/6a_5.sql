...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Belgium,Denmark,Greece,"Hong Kong",Ireland,Netherlands,Philippines,Portugal,"South Korea","Soviet Union",Sweden,"West Germany"}'::text[])) AND (mi1.info = ANY ('{Belgium,Denmark,Greece,"Hong Kong",Ireland,Netherlands,Philippines,Portugal,"South Korea","Soviet Union",Sweden,"West Germany"}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = ANY ('{A5236,A5352,B6163,C6235,C6456,H4521,J5245,S1524,S2516,S5362}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('Belgium','Denmark','Greece','Hong Kong','Ireland','Netherlands','Philippines','Portugal','South Korea','Soviet Union','Sweden','West Germany')) 
 AND (it1.id IN ('2','8')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
 ...
 AND (n.name_pcode_nf in ('A5236','A5352','B6163','C6235','C6456','H4521','J5245','S1524','S2516','S5362')) 
