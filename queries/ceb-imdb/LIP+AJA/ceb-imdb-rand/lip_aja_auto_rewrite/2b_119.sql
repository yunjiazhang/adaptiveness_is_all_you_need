...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 18) AND (mi1.info = 'Metro-Goldwyn-Mayer Studios - 10202 W. Washington Blvd., Culver City, California, USA'::text)) AND ((mi1.info_type_id = 18) AND (mi1.info = 'Metro-Goldwyn-Mayer Studios - 10202 W. Washington Blvd., Culver City, California, USA'::text));
SELECT sum(pg_lip_bloom_add(2, mi2.movie_id)), sum(pg_lip_bloom_add(3, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 5) AND (mi2.info = 'USA:Approved'::text)) AND ((mi2.info_type_id = 5) AND (mi2.info = 'USA:Approved'::text));
...
WHERE 
 ...
 AND (k.keyword IN ('anal-sex','blood','father-daughter-relationship','flashback','homosexual','independent-film','jealousy','lesbian','male-nudity','mother-daughter-relationship','mother-son-relationship','murder','nudity','one-word-title','party','sequel','violence')) 
