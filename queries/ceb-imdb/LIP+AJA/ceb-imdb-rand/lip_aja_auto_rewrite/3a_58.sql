...
SELECT sum(pg_lip_bloom_add(0, cn.id)) FROM company_name AS cn 
        WHERE ((cn.country_code)::text = ANY ('{[bg],[cu],[cz],[il],[my],[sg],[th]}'::text[]));
...
WHERE 
 ...
 AND (k.keyword IN ('bare-breasts','cigarette-smoking','flashback','gun','hardcore','interview','lesbian-sex','murder')) 
 AND (cn.country_code IN ('[bg]','[cu]','[cz]','[il]','[my]','[sg]','[th]')) 
