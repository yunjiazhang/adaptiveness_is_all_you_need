...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"Dolby Digital",Dolby}'::text[])) AND (mi1.info_type_id = 6)) AND ((mi1.info = ANY ('{"Dolby Digital",Dolby}'::text[])) AND (mi1.info_type_id = 6));
...
WHERE 
 ...
 AND (mi1.info IN ('Dolby Digital','Dolby')) 
 AND (it1.id IN ('6')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
