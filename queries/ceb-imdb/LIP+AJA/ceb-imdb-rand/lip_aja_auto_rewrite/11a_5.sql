...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%u%'::text) AND (mi1.info_type_id = 8)) AND ((mi1.info ~~* '%u%'::text) AND (mi1.info_type_id = 8));
...
WHERE 
 ...
 AND (kt.kind ILIKE '%m%') 
 AND (rt.role IN ('actor','actress','costume designer','director','miscellaneous crew','producer','production designer','writer')) 
