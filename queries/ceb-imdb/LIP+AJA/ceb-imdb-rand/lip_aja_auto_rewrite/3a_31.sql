...
SELECT sum(pg_lip_bloom_add(0, cn.id)) FROM company_name AS cn 
        WHERE ((cn.country_code)::text = ANY ('{[nl],[suhh]}'::text[]));
...
WHERE 
 ...
 AND (k.keyword IN ('anal-sex','based-on-novel','based-on-play','family-relationships','hospital','male-frontal-nudity','marriage','mother-daughter-relationship','number-in-title','suicide','tv-mini-series','violence')) 
 AND (cn.country_code IN ('[nl]','[suhh]')) 
