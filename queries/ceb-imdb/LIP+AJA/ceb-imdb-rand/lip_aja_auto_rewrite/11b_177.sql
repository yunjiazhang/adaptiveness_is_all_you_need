...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%c%'::text) AND (mi1.info_type_id = 5)) AND ((mi1.info ~~* '%c%'::text) AND (mi1.info_type_id = 5));
...
WHERE 
 ...
 AND (kt.kind IN ('episode','tv mini series','tv movie','tv series','video game')) 
 AND (rt.role IN ('cinematographer','composer','editor','guest','writer')) 
