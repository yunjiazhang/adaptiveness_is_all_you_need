...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 6) AND (mi1.info = ANY ('{"Dolby Digital",Mono,SDDS,Stereo}'::text[]))) AND ((mi1.info_type_id = 6) AND (mi1.info = ANY ('{"Dolby Digital",Mono,SDDS,Stereo}'::text[])));
...
WHERE 
 ...
 AND (it1.id IN ('6')) 
 AND (mi1.info in ('Dolby Digital','Mono','SDDS','Stereo')) 
 AND (kt.kind in ('episode','movie')) 
 AND (rt.role in ('actor','miscellaneous crew')) 
 AND (n.gender in ('m')) 
 AND (n.surname_pcode in ('B4','D5','G65','H2','H63','J52','K5','M2','M635','P62','S35','S5','T46','W452') OR n.surname_pcode IS NULL) 
