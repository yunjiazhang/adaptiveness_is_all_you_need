...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{30,USA:60}'::text[])) AND (mi1.info_type_id = 1)) AND ((mi1.info = ANY ('{30,USA:60}'::text[])) AND (mi1.info_type_id = 1));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND (((n.name_pcode_nf)::text = ANY ('{D5162,J5215}'::text[])) OR (n.name_pcode_nf IS NULL)));
...
WHERE 
 ...
 AND (n.name_pcode_nf in ('D5162','J5215') OR n.name_pcode_nf IS NULL) 
