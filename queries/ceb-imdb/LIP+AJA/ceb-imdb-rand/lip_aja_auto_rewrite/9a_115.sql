...
WHERE 
 ...
 AND (it1.id IN ('16')) 
 AND (it2.id IN ('21')) 
 AND (mi1.info ILIKE '%ur%') 
 AND (pi.info ILIKE '%18%') 
 AND (kt.kind IN ('tv mini series','tv movie','video game','video movie')) 
 AND (rt.role IN ('actor','actress','cinematographer','composer','director','editor','miscellaneous crew','production designer','writer')) 
