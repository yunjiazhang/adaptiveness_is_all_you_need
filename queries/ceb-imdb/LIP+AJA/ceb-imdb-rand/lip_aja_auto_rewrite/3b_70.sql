...
SELECT sum(pg_lip_bloom_add(0, n.id)) FROM name AS n 
        WHERE ((n.name_pcode_nf)::text ~~* '%a15%'::text);
...
WHERE 
 ...
 AND (t.title ILIKE '%hu%') 
 AND (n.name_pcode_nf ILIKE '%a15%') 
 AND (cn.name ILIKE '%d%') 
 AND (kt.kind IN ('episode','tv movie','tv series','video movie')) 
 AND (rt.role IN ('actor','actress','cinematographer','director','editor','producer','production designer','writer')) 
