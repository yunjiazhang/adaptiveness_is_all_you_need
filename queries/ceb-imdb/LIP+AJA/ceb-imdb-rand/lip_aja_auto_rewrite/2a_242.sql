...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 3) AND (mi2.info = ANY ('{Comedy,Drama,Horror,Short,Thriller}'::text[])));


/*+
NestLoop(mi1 t kt mi2 it1 it2 mk ci rt n k)
HashJoin(mi1 t kt mi2 it1 it2 mk ci rt n)
...
WHERE 
 ...
 AND (mi1.info in ('Australia:M','Finland:K-16','Iceland:L','Sweden:Btl','UK:U','USA:Approved','USA:PG','USA:R','West Germany:16')) 
 AND (mi2.info in ('Comedy','Drama','Horror','Short','Thriller')) 
