...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Drama,Family,Fantasy,Romance,Talk-Show,Western}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('Action','Drama','Family','Fantasy','Romance','Talk-Show','Western')) 
 AND (mi2.info IN ('Hungary','Italy','Morocco','Pakistan','West Germany')) 
