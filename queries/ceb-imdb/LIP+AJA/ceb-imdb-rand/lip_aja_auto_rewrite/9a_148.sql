...
SELECT sum(pg_lip_bloom_add(0, pi.person_id)) FROM person_info AS pi 
        WHERE ((pi.info ~~* '%te%'::text) AND (pi.info_type_id = 38));


/*+
NestLoop(it2 it1 mi1 t ci rt pi n kt)
NestLoop(it1 mi1 t ci rt pi n kt)
NestLoop(mi1 t ci rt pi n kt)
...
WHERE 
 ...
 AND (it1.id IN ('16')) 
 AND (it2.id IN ('38')) 
 AND (mi1.info ILIKE '%taiw%') 
 AND (pi.info ILIKE '%te%') 
 AND (kt.kind IN ('episode','movie','tv mini series','tv movie','tv series','video game','video movie')) 
 AND (rt.role IN ('actress','cinematographer','composer','costume designer','director','editor','guest','production designer','writer')) 
