...
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.name_pcode_cf)::text = ANY ('{P5235,P6252}'::text[])) AND ((n.gender)::text = 'f'::text));
...
WHERE 
 ...
 AND (cn.name in ('Columbia Broadcasting System (CBS)','Metro-Goldwyn-Mayer (MGM)','Paramount Pictures','Pathé Frères','Warner Home Video')) 
