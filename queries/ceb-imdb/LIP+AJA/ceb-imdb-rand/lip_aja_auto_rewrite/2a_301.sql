...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Documentary,Drama,Sci-Fi,Short}'::text[])));
...
WHERE 
 ...
 AND (mi1.info in ('Documentary','Drama','Sci-Fi','Short')) 
 AND (mi2.info in ('Dolby','Mono','Stereo')) 
