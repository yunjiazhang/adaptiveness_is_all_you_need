...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%bahr%'::text) AND (mi1.info_type_id = 16)) AND ((mi1.info ~~* '%bahr%'::text) AND (mi1.info_type_id = 16));


/*+
NestLoop(pi n ci rt t kt mi1 it1 it2)
...
WHERE 
 ...
 AND (rt.role IN ('actor','actress','director','miscellaneous crew','production designer')) 
