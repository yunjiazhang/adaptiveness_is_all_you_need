...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%ofm:%'::text) AND (mi1.info_type_id = 7)) AND ((mi1.info ~~* '%ofm:%'::text) AND (mi1.info_type_id = 7));
...
WHERE 
 ...
 AND (rt.role IN ('actress','costume designer','director','editor','guest','miscellaneous crew','writer')) 
