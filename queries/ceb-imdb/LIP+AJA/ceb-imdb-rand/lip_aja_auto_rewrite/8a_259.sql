...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{"RAT:1.33 : 1","RAT:1.78 : 1"}'::text[])) AND (mi1.info_type_id = 7)) AND ((mi1.info = ANY ('{"RAT:1.33 : 1","RAT:1.78 : 1"}'::text[])) AND (mi1.info_type_id = 7));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = ANY ('{f,m}'::text[])) OR (n.gender IS NULL)) AND ((n.surname_pcode)::text = ANY ('{A45,B65,C4,C65,F652,G6,H52,J52,L2,L5,L52,M24,P6,W3}'::text[])));
