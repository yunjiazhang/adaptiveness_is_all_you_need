...
WHERE 
 ...
 AND (it1.id IN ('16')) 
 AND (it2.id IN ('21')) 
 AND (mi1.info ILIKE '%uk:2%') 
 AND (pi.info ILIKE '%23%') 
 AND (kt.kind IN ('episode','movie','tv movie','video game','video movie')) 
 AND (rt.role IN ('actor','actress','composer','costume designer','director','editor','guest','miscellaneous crew','producer','production designer','writer')) 
