...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Comedy,Documentary,Short}'::text[]))) AND ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Comedy,Documentary,Short}'::text[])));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.surname_pcode)::text = ANY ('{B42,B6,B62,B65,H2,L,L2,L5,S53,W3}'::text[])));
