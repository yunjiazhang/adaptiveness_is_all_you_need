...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%sci%'::text) AND (mi1.info_type_id = 3)) AND ((mi1.info ~~* '%sci%'::text) AND (mi1.info_type_id = 3));
...
WHERE 
 ...
 AND (rt.role IN ('actor','actress','cinematographer','composer','costume designer','director','editor','miscellaneous crew','producer','production designer')) 
 AND (t.production_year <= 2015) 
 AND (t.production_year >= 1990) 
 AND (it1.id IN ('3')) 
 AND (mi1.info ILIKE '%sci%') 
 AND (pi.info ILIKE '%mark%') 
 AND (it2.id IN ('38')) 
