...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info ~~* '%y%'::text) AND (mi1.info_type_id = 8)) AND ((mi1.info ~~* '%y%'::text) AND (mi1.info_type_id = 8));
...
WHERE 
 ...
 AND (kt.kind IN ('episode','movie','tv mini series','tv series','video game')) 
 AND (rt.role IN ('actor','actress','cinematographer')) 
 AND (t.production_year <= 2015) 
 AND (t.production_year >= 1875) 
 AND (it1.id IN ('8')) 
 AND (mi1.info ILIKE '%y%') 
 AND (pi.info ILIKE '%marg%') 
 AND (it2.id IN ('26')) 
