...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Adventure,Animation,Comedy,Crime,Documentary,Drama,Horror,Music,Musical,Romance,Western}'::text[])));
...
WHERE 
 ...
 AND (mi1.info in ('Action','Adventure','Animation','Comedy','Crime','Documentary','Drama','Horror','Music','Musical','Romance','Western')) 
