...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Comedy,Documentary,Drama,Mystery,Sci-Fi,War}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Animation','Comedy','Documentary','Drama','Mystery','Sci-Fi','War') 
 AND mi2.info IN ('Czech','Filipino','German','Malayalam','Polish','Spanish') 
