...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Animation,Comedy,Crime,Documentary,Drama}'::text[])) AND (mi1.info = ANY ('{Animation,Comedy,Crime,Documentary,Drama}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, pi1.person_id)), sum(pg_lip_bloom_add(4, pi1.person_id)) FROM person_info AS pi1 
        WHERE (pi1.info_type_id = 26) AND (pi1.info_type_id = 26);
...
NestLoop(mii2 mii1 t mk ci rt an n)
...
WHERE 
 ...
 AND (mi1.info IN ('Animation','Comedy','Crime','Documentary','Drama')) 
