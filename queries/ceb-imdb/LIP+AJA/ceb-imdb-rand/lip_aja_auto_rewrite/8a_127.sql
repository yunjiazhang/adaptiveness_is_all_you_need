...
WHERE 
 ...
 AND (it1.id IN ('3')) 
 AND (mi1.info in ('Adventure','Fantasy','Music','Musical','Mystery','Sci-Fi','War')) 
 AND (kt.kind in ('movie')) 
 AND (rt.role in ('actress','cinematographer','miscellaneous crew')) 
 AND (n.gender in ('f') OR n.gender IS NULL) 
 AND (n.surname_pcode in ('L','R2','S53') OR n.surname_pcode IS NULL) 
 AND (t.production_year <= 1975) 
 AND (t.production_year >= 1875) 
 AND (cn.name in ('Metro-Goldwyn-Mayer (MGM)','Paramount Pictures','Warner Home Video')) 
