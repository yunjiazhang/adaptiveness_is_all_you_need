...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"OFM:16 mm",OFM:Video,"PCS:Digital Intermediate",PCS:Spherical,"PFM:70 mm","RAT:1.66 : 1","RAT:2.35 : 1"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"OFM:16 mm",OFM:Video,"PCS:Digital Intermediate",PCS:Spherical,"PFM:70 mm","RAT:1.66 : 1","RAT:2.35 : 1"}'::text[])));
SELECT sum(pg_lip_bloom_add(2, mi2.movie_id)), sum(pg_lip_bloom_add(3, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{Germany,India,Italy,Japan,UK,USA,"West Germany"}'::text[]))) AND ((mi2.info_type_id = 8) AND (mi2.info = ANY ('{Germany,India,Italy,Japan,UK,USA,"West Germany"}'::text[])));
