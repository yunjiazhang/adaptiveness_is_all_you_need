...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 7) AND (mi2.info = ANY ('{"MET:15.2 m","MET:150 m","MET:23 m","MET:30 m","MET:600 m","OFM:35 mm",PCS:Spherical,"RAT:1.33 : 1"}'::text[])));
SELECT sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Adventure,Crime,Documentary,Fantasy,Short,War,Western}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Adventure','Crime','Documentary','Fantasy','Short','War','Western') 
 AND mi2.info IN ('MET:15.2 m','MET:150 m','MET:23 m','MET:30 m','MET:600 m','OFM:35 mm','PCS:Spherical','RAT:1.33 : 1') 
