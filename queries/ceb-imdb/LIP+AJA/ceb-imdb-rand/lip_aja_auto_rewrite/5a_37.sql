...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"OFM:35 mm",PCS:Spherical,"PFM:35 mm","RAT:1.37 : 1"}'::text[]))) AND ((mi1.info_type_id = 7) AND (mi1.info = ANY ('{"OFM:35 mm",PCS:Spherical,"PFM:35 mm","RAT:1.37 : 1"}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('OFM:35 mm','PCS:Spherical','PFM:35 mm','RAT:1.37 : 1')) 
 AND (it1.id IN ('7')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
