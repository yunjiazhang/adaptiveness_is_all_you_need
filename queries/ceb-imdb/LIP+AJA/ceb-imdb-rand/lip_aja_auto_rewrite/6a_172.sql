...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{"Dolby Digital","Dolby SR",Dolby,Silent}'::text[])) AND (mi1.info = ANY ('{"Dolby Digital","Dolby SR",Dolby,Silent}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND ((n.name_pcode_nf)::text = ANY ('{A52,A5243,A532,C4316,C6456,D525,G1642,M2345,M6165,M6456,M6562}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('Dolby Digital','Dolby SR','Dolby','Silent')) 
 AND (it1.id IN ('2','6','90')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
 ...
 AND (n.gender in ('f')) 
 AND (n.name_pcode_nf in ('A52','A5243','A532','C4316','C6456','D525','G1642','M2345','M6165','M6456','M6562')) 
 AND (ci.note IS NULL) 
