...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Australia:G,Australia:M,"Buenos Aires, Federal District, Argentina","New York City, New York, USA",UK:15}'::text[])) AND (mi1.info = ANY ('{Australia:G,Australia:M,"Buenos Aires, Federal District, Argentina","New York City, New York, USA",UK:15}'::text[]));
...
WHERE 
 ...
 AND (mi1.info IN ('Australia:G','Australia:M','Buenos Aires, Federal District, Argentina','New York City, New York, USA','UK:15')) 
 AND (it1.id IN ('18','5','98')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
