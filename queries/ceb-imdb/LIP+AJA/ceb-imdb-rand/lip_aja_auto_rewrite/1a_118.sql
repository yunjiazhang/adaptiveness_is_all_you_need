...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Biography,Comedy,Crime,Documentary,Drama,Fantasy,Music,Romance}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Action','Biography','Comedy','Crime','Documentary','Drama','Fantasy','Music','Romance') 
 AND mi2.info IN ('Finnish','French','Hindi','Italian','Polish','Russian','Spanish') 
