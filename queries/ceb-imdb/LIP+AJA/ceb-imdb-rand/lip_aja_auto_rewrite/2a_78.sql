...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Action,Comedy,Crime,Family,Romance,Thriller,War}'::text[])));
...
WHERE 
 ...
 AND (mi1.info in ('Action','Comedy','Crime','Family','Romance','Thriller','War')) 
 AND (mi2.info in ('Australia:G','Australia:M','Finland:K-16','Iceland:16','Sweden:15','UK:15','USA:Approved','West Germany:16')) 
