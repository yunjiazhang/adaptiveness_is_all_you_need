...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Datasat,"Dolby Digital EX","Dolby SR",Dolby,SDDS,"Ultra Stereo"}'::text[])) AND (mi1.info = ANY ('{Datasat,"Dolby Digital EX","Dolby SR",Dolby,SDDS,"Ultra Stereo"}'::text[]));
