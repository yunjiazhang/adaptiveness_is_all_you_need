...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{60,OFM:Live,OFM:Video,PFM:Video}'::text[])) AND (mi1.info = ANY ('{60,OFM:Live,OFM:Video,PFM:Video}'::text[]));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND ((n.name_pcode_nf)::text = ANY ('{B6161,J3261,R3565}'::text[])));
...
WHERE 
 ...
 AND (n.gender in ('f')) 
 AND (n.name_pcode_nf in ('B6161','J3261','R3565')) 
 AND (ci.note IS NULL) 
