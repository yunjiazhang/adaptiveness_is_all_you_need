...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Adventure,Biography,Fantasy,History,Romance,Sci-Fi,Sport,USA:2005}'::text[])) AND (mi1.info = ANY ('{Adventure,Biography,Fantasy,History,Romance,Sci-Fi,Sport,USA:2005}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.name_pcode_nf)::text = ANY ('{C6235,E4213}'::text[])) OR (n.name_pcode_nf IS NULL)) AND ((n.gender)::text = 'f'::text));
...
WHERE 
 ...
 AND (mi1.info IN ('Adventure','Biography','Fantasy','History','Romance','Sci-Fi','Sport','USA:2005')) 
 AND (it1.id IN ('104','16','3')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
