...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)), sum(pg_lip_bloom_add(1, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 5) AND (mi2.info = ANY ('{Argentina:Atp,Finland:K-16,Sweden:15,UK:U,USA:Approved,USA:Passed}'::text[]))) AND ((mi2.info_type_id = 5) AND (mi2.info = ANY ('{Argentina:Atp,Finland:K-16,Sweden:15,UK:U,USA:Approved,USA:Passed}'::text[])));
SELECT sum(pg_lip_bloom_add(2, mi1.movie_id)), sum(pg_lip_bloom_add(3, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Adventure,Animation,Comedy,Drama,Family,Music,Romance,Western}'::text[]))) AND ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Adventure,Animation,Comedy,Drama,Family,Music,Romance,Western}'::text[])));
