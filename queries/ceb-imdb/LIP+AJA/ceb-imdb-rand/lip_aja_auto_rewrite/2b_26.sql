...
SELECT sum(pg_lip_bloom_add(2, mi1.movie_id)), sum(pg_lip_bloom_add(3, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{Comedy,Thriller}'::text[])) AND (mi1.info_type_id = 3)) AND ((mi1.info = ANY ('{Comedy,Thriller}'::text[])) AND (mi1.info_type_id = 3));
...
WHERE 
 ...
 AND (mi1.info in ('Comedy','Thriller')) 
 ...
 AND (k.keyword IN ('bare-breasts','bare-chested-male','death','friendship','gay','hardcore','hospital','independent-film','jealousy','love','marriage','non-fiction','one-word-title','party','sequel','sex','singer','surrealism','tv-mini-series','violence')) 
