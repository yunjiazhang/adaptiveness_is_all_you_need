...
SELECT sum(pg_lip_bloom_add(0, pi.person_id)) FROM person_info AS pi 
        WHERE ((pi.info ~~* '%usa%'::text) AND (pi.info_type_id = 20));
...
WHERE 
 ...
 AND (rt.role IN ('actress','cinematographer','composer','editor','guest','miscellaneous crew','producer','production designer','writer')) 
