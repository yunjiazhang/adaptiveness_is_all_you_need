...
SELECT sum(pg_lip_bloom_add(0, pi.person_id)) FROM person_info AS pi 
        WHERE ((pi.info ~~* '%j%'::text) AND (pi.info_type_id = 24));
...
WHERE 
 ...
 AND (kt.kind IN ('movie','tv series','video game')) 
 AND (rt.role IN ('actor','composer','costume designer','director','miscellaneous crew','producer','production designer')) 
