...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info = ANY ('{USA:30,USA:60}'::text[])) AND (mi1.info_type_id = 1)) AND ((mi1.info = ANY ('{USA:30,USA:60}'::text[])) AND (mi1.info_type_id = 1));
SELECT sum(pg_lip_bloom_add(2, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'f'::text) AND ((n.name_pcode_nf)::text = ANY ('{B6161,C6235,J5162,K3451,K6235}'::text[])));
