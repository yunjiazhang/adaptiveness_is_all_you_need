...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 3) AND (mi2.info = ANY ('{Action,Animation,Comedy,Crime,Documentary,Drama,Musical,Short,Western}'::text[])));
...
WHERE 
 ...
 AND (mi2.info in ('Action','Animation','Comedy','Crime','Documentary','Drama','Musical','Short','Western')) 
