...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Comedy,Crime,Documentary,Drama,Romance,Short}'::text[]))) AND ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Animation,Comedy,Crime,Documentary,Drama,Romance,Short}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Animation','Comedy','Crime','Documentary','Drama','Romance','Short') 
