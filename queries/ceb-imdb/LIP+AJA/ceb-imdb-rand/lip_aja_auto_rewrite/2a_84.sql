...
SELECT sum(pg_lip_bloom_add(0, mi2.movie_id)) FROM movie_info AS mi2 
        WHERE ((mi2.info_type_id = 3) AND (mi2.info = ANY ('{Comedy,Crime,Drama,Short}'::text[])));
...
WHERE 
 ...
 AND (mi1.info in ('Argentina','Brazil','France','Italy','Japan','Mexico','Philippines','Sweden')) 
 AND (mi2.info in ('Comedy','Crime','Drama','Short')) 
