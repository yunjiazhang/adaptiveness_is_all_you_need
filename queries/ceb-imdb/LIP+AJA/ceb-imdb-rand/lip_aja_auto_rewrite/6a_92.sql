...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Hindi,Mandarin,Portuguese,Russian,Tagalog}'::text[])) AND (mi1.info = ANY ('{Hindi,Mandarin,Portuguese,Russian,Tagalog}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE (((n.gender)::text = 'm'::text) AND ((n.name_pcode_nf)::text = ANY ('{A4152,A5361,B6514,F6532,J25,J5241,L2142,M2423,M6245,M6354,W4525}'::text[])));
...
WHERE 
 ...
 AND (mi1.info IN ('Hindi','Mandarin','Portuguese','Russian','Tagalog')) 
 AND (it1.id IN ('107','4')) 
 AND it3.id = '100' 
 AND it4.id = '101' 
 ...
 AND (n.gender in ('m')) 
 AND (n.name_pcode_nf in ('A4152','A5361','B6514','F6532','J25','J5241','L2142','M2423','M6245','M6354','W4525')) 
 AND (ci.note IS NULL) 
