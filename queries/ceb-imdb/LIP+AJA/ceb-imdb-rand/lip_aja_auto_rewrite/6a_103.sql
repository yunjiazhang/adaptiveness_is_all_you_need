...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)), sum(pg_lip_bloom_add(1, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE (mi1.info = ANY ('{Action,Adventure,Family,Musical,Thriller,War,Western}'::text[])) AND (mi1.info = ANY ('{Action,Adventure,Family,Musical,Thriller,War,Western}'::text[]));
...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n 
        WHERE ((((n.gender)::text = 'f'::text) OR (n.gender IS NULL)) AND ((n.name_pcode_nf)::text = 'M6263'::text));
...
WHERE 
 ...
 AND (mi1.info IN ('Action','Adventure','Family','Musical','Thriller','War','Western')) 
