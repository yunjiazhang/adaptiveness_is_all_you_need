...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Comedy,Documentary,Horror,Thriller}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Comedy','Documentary','Horror','Thriller') 
 AND mi2.info IN ('Italian','Japanese','Russian','Swedish') 
