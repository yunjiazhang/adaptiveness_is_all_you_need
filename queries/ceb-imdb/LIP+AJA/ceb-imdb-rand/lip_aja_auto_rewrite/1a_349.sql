...
SELECT sum(pg_lip_bloom_add(0, mi1.movie_id)) FROM movie_info AS mi1 
        WHERE ((mi1.info_type_id = 3) AND (mi1.info = ANY ('{Biography,Crime,Drama}'::text[])));
...
WHERE 
 ...
 AND mi1.info IN ('Biography','Crime','Drama') 
 AND mi2.info IN ('LAB:DeLuxe, Hollywood (CA), USA','OFM:16 mm','OFM:35 mm') 
