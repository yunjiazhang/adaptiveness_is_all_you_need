...
class Balsa_CEBIMDBRandSplit(Balsa_JOBRandSplit):
    ...
    def Params(self):
        ...
        # import random
        # random.seed(seed)
        # sampled_train_queries = random.sample(all_queries, sampled_train_query_num)
        # p.query_glob = sampled_train_queries
        p.query_glob = ['*.sql']
        p.test_query_glob = ['t*.sql']
        p.epochs = 2
        p.skip_training_on_expert = False
...
@balsa.params_registry.Register
class Balsa_CEBIMDBRandSplit_Sample(Balsa_JOBRandSplit):
    ...
    def Params(self):
        ...
        p.query_dir = '/nobackup/yunjia/adaptiveness_vs_learning/queries/ceb-imdb/original/ceb-imdb-rand-balsa'
        all_queries = os.listdir(p.query_dir)
        sampled_total_query_num = 600
        
        import random
        random_seed = 0
        random.seed(random_seed)
        
        sampled_train_queries = random.sample(all_queries, sampled_total_query_num)
        p.query_glob = sampled_train_queries
        # p.query_glob = ['*.sql']
...
@balsa.params_registry.Register
class Balsa_CEBIMDBSlowSplit_Sample(Balsa_JOBSlowSplit):
