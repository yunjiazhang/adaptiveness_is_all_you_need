...
class BaselineStats(Baseline):
    ...
    def Params(self):
        p = super().Params()
        p.query_glob = ['*.sql']
        p.query_dir = '/mnt/adaptiveness_vs_learning/queries/stats/original/all/'
...
########################## Stats config ##########################
@balsa.params_registry.Register
class Balsa_StatsRand_Baseline(BalsaParams):
...
@balsa.params_registry.Register  # keep
class Balsa_StatsRand(Balsa_JOBSlowSplit):
    def Params(self):
        p = super().Params()
        original_query_dir = '/mnt/adaptiveness_vs_learning/queries/stats/original/balsa_stats_rand/'
        p.query_dir = original_query_dir
        
        p.query_glob = [q \
            for q in os.listdir(original_query_dir)
            if '.sql' in q]

        test_queries = [q \
            for q in os.listdir(original_query_dir) \
            if '.sql' in q and 't' in q]
        p.test_query_glob = test_queries

        p.query_glob += test_queries
        p.query_glob = list(set(p.query_glob))
        
        # p.sim_checkpoint = './checkpoints/sim-Stack-epoch=49.ckpt'
        p.sim_checkpoint = None
        p.skip_training_on_expert = False
        p.epochs = 100
        p.sim = True
        p.initial_timeout_ms = 1200000
        # p.recorded_per_query_timeout_ms_file = './data/recorded_per_query_timeout_ms_file.json'
...
@balsa.params_registry.Register  # keep
class Balsa_StatsSlow(Balsa_JOBSlowSplit):
    def Params(self):
        p = super().Params()
        original_query_dir = '/mnt/adaptiveness_vs_learning/queries/stats/original/balsa_stats_slow/'
        p.query_dir = original_query_dir
